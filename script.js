...
async function readSoftwareTarget(file) {
    const response = await fetch(file.download_url);
    if (!response.ok) {
        throw new Error(`软件链接读取失败：${response.status}`);
    }
    ...
    return firstValidLine?.startsWith('http') ? firstValidLine : file.download_url;
}
...
async function renderSoftwareItems(items, path, container) {
    for (const item of items) {
        ...
        const targetUrl = await readSoftwareTarget(item);
